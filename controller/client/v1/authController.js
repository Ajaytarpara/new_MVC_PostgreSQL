...
const login = async (req,res)=>{
  try {
    ...
    let result = await authService.loginUser(username,password,authConstant.PLATFORM.CLIENT); 
    if (result.flag){
...
const logout = async (req, res) => {
  try {
    let userTokens = await dbService.findOne(userTokens, {
    ...
    await dbService.update(userTokens,{ id:id }, userTokens.toJSON());
