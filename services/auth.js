...
const changePassword = async (params)=>{
  try {
    ...
    let updatedUser = dbService.update(model.user,{ id :user.id },{ password:password });
    if (!updatedUser) {
      return {
        flag:true,
...
const resetPassword = async (userId, newPassword) => {
  try {
    ...
    let updatedUser = await dbService.update(model.user, { id :userId }, { password: newPassword });
    if (!updatedUser) {
      return {
        flag: true,
